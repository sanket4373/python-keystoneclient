...
def cms_hash_data(data, mode='md5'):
        """ 
            Hash Input Data
            If input mode is not specified then default to md5.
...
#########################
def cms_encrypt_data(data, encryption_key, iv='This is an IV456'):
        cipher = AES.new(encryption_key, AES.MODE_CFB, iv)

	# Pack the input Data to prepare for encoding
	packed_data = msgpack.packb(data)

	# Now Encrypt the data
	encrypted_quoted_data = cipher.encrypt(packed_data)

	# Now quote the Encrypted Data so that it can be passed as a string in HTTP messages.
        return urllib.parse.quote(encrypted_quoted_data)

#########################
# Data Decryption Method 
#########################
def cms_decrypt_data(encrypted_data, decryption_key, iv='This is an IV456'):
        cipher = AES.new(decryption_key, AES.MODE_CFB, iv)

	# Unquote input quoted and encrypted data
	unquoted_encrypted_data = urllib.parse.unquote(six.binary_type(encrypted_data))

	# Decrypt the data now
	decrypted_data = cipher.decrypt(unquoted_encrypted_data)
	
 	# Now unpack the decrypted packet to get the original data back
