...
class AuthProtocol(object):
	...
	start_time = time.time()
	   ...
	   # START Kerberos Processing
	    #kerb_service_tgt = self._get_header(env, 'X-Kerb-Serv-Tgt')
	    #kerb_service_timestamp = self._get_header(env, 'X-Kerb-Serv-Timestamp')
	    #timetaken = (time.time() - start_time) * 1000
	    #print timetaken
	    #print "######################################"
	    #print "My Kerberos Service TGT is ", kerb_service_tgt
	    #print "My Kerberos Service Timestamp is ", kerb_service_timestamp
	    #Kos = "Service_SessionKey"
	    #Kos_key = cms.cms_hash_data(Kos)
	
  
	    # Decrypting the received Service_TGT using the ServiceKey which should be already stored by Nova
	    # Generally all the services should have the ServiceKey prior stored with them
	    #service_tgt = cms.cms_decrypt_data(kerb_service_tgt, Kos_key)
       	    #print ("Decrypted Service TGT -> ", service_tgt)
 
	    # Decrypting the received Service_Timestamp using the User_Service_SessionKey
  	    #Kaos_key  = service_tgt['User_Service_SessionKey'] 
	    #service_timestamp = cms.cms_decrypt_data(kerb_service_timestamp, Kaos_key)
	    #print ("Decrypted Service TimeStamp ", service_timestamp)

	    #print "######################################"
	    # End Kerberos Processing 
